#ifndef __Tensor2DFromCluster2D_CXX__
#define __Tensor2DFromCluster2D_CXX__

#include "Tensor2DFromCluster2D.h"
#include "larcv/core/DataFormat/EventVoxel2D.h"
namespace larcv {

  static Tensor2DFromCluster2DProcessFactory
  __global_Tensor2DFromCluster2DProcessFactory__;

  Tensor2DFromCluster2D::Tensor2DFromCluster2D(const std::string name)
    : ProcessBase(name) {}

  void Tensor2DFromCluster2D::configure_labels(const PSet& cfg)
  {
  ...
  void Tensor2DFromCluster2D::configure(const PSet& cfg) {
    ...
    if (_fixed_pi_v.empty()) {
      ...
      _fixed_pi_v.resize(_cluster2d_producer_v.size(), fixed_pi);
    } else if (_fixed_pi_v.size() != _cluster2d_producer_v.size()) {
    ...
    if (_pi_type_v.empty()) {
      ...
      _pi_type_v.resize(_cluster2d_producer_v.size(), pi_type);
    } else if (_pi_type_v.size() != _cluster2d_producer_v.size()) {
      LARCV_CRITICAL() << "PITypeList size mismatch with other input parameters!" << std::endl;
      throw larbys();
    }
  }

  void Tensor2DFromCluster2D::initialize() {}

  bool Tensor2DFromCluster2D::process(IOManager& mgr) {
    ...
    if (_cluster2d_producer_v.empty()) {
      ...
      producer_list = _cluster2d_producer_v;
  ...
  void Tensor2DFromCluster2D::finalize() {}
}
#endif
