...
bool BBoxFromClusterVoxel3D::process(IOManager& mgr) {
  ...
  for (size_t i_part = 0; i_part < ev_part_input.as_vector().size(); i_part++) {
    auto _orig_particle = ev_part_input.as_vector().at(i_part);
    Particle _new_particle(_orig_particle.shape());
    // Set values for the new particle:
    _new_particle.id(_orig_particle.id());
    _new_particle.mcst_index(_orig_particle.mcst_index());
    _new_particle.mct_index(_orig_particle.mct_index());
    _new_particle.nu_current_type(_orig_particle.nu_current_type());
    _new_particle.nu_interaction_type(_orig_particle.nu_interaction_type());
    _new_particle.track_id(_orig_particle.track_id());
    _new_particle.pdg_code(_orig_particle.pdg_code());
    _new_particle.momentum(_orig_particle.px(), _orig_particle.py(),
                           _orig_particle.pz());

    _new_particle.position(_orig_particle.position());
    _new_particle.end_position(_orig_particle.end_position());
    _new_particle.first_step(_orig_particle.first_step());
    _new_particle.last_step(_orig_particle.last_step());
    _new_particle.distance_travel(_orig_particle.distance_travel());
    _new_particle.energy_init(_orig_particle.energy_init());
    _new_particle.energy_deposit(_orig_particle.energy_deposit());
    _new_particle.creation_process(_orig_particle.creation_process());
    ...
    _new_particle.boundingbox_3d(BBox3D(x_min, y_min, z_min, x_max, y_max, z_max));

    // XY projection: 
    if (_xy){
      _new_particle.boundingbox_2d(BBox2D(x_min, y_min, x_max, y_max), 0);
    }
    // YZ projection: 
    if (_yz){
      _new_particle.boundingbox_2d(BBox2D(y_min, z_min, y_max, z_max), 1);
    }
    // ZX projection: 
    if (_zx){
      _new_particle.boundingbox_2d(BBox2D(z_min, x_min, z_max, x_max), 2);
    }

    ev_part_output.emplace_back(std::move(_new_particle));
