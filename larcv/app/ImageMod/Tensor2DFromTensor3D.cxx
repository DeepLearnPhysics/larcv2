...
bool Tensor2DFromTensor3D::process(IOManager& mgr) {
  ...
  for (auto const& vox : ev_tensor3d.as_vector()) {
    ...
    if (_xy) {
      ...
      vs_xy.emplace(meta_xy.index(meta_xy.row(pos.y), meta_xy.col(pos.x)),
                    vox.value(), true);
    }
    if (_yz) {
      LARCV_DEBUG() << "(y,z) = (" << pos.y << "," << pos.z << ")" << std::endl
                    << meta_yz.dump() << std::endl;
      vs_yz.emplace(meta_yz.index(meta_yz.row(pos.z), meta_yz.col(pos.y)),
                    vox.value(), true);
    }
    if (_zx) {
      LARCV_DEBUG() << "(z,x) = (" << pos.z << "," << pos.x << ")" << std::endl
                    << meta_zx.dump() << std::endl;
      vs_zx.emplace(meta_zx.index(meta_zx.row(pos.x), meta_zx.col(pos.z)),
                    vox.value(), true);
    }
  }

  auto& out_tensor2d =
      mgr.get_data<larcv::EventSparseTensor2D>(_output_producer);
