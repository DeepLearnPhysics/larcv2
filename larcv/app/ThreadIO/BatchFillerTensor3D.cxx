...
namespace larcv {
  ...
  static BatchFillerTensor3DProcessFactory __global_BatchFillerTensor3DProcessFactory__;
  ...
  void BatchFillerTensor3D::finalize()
  { _entry_data.clear(); }

  void BatchFillerTensor3D::assert_dimension(const EventSparseTensor3D& voxel_data) const
  {
    auto const& voxel_meta = voxel_data.meta();
    if(_nx != voxel_meta.num_voxel_x()) {
      LARCV_CRITICAL() << "# of X-voxels (" << _nx << ") changed ... now " << voxel_meta.num_voxel_x() << std::endl;
      throw larbys();
    }
    if(_ny != voxel_meta.num_voxel_y()) {
      LARCV_CRITICAL() << "# of Y-voxels (" << _ny << ") changed ... now " << voxel_meta.num_voxel_y() << std::endl;
      throw larbys();
    }
    if(_nz != voxel_meta.num_voxel_z()) {
      LARCV_CRITICAL() << "# of Z-voxels (" << _nz << ") changed ... now " << voxel_meta.num_voxel_z() << std::endl;
      throw larbys();
    }
    return;
  }
  ...
  {
    LARCV_DEBUG() <<"start"<<std::endl;
    auto const& voxel_data = mgr.get_data<larcv::EventSparseTensor3D>(_tensor3d_producer);
    if(voxel_data.as_vector().empty()) {
      LARCV_CRITICAL() << "Could not locate non-empty voxel data w/ producer name " << _tensor3d_producer << std::endl;
      throw larbys();
    }
